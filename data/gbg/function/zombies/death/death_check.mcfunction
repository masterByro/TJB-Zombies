...
execute unless entity @a[gamemode=!spectator,tag=!Downed] run function gbg:zombies/rounds/stop
execute if entity @a[tag=!Downed,gamemode=!spectator] as @a[scores={NewDeath=1..}] run function gbg:zombies/death/death_downed

# Downed players
execute as @a[tag=Downed] at @s run function gbg:zombies/death/death_tick