...
scoreboard players operation @s gbg.temp %= 2 number
execute if score @s gbg.temp matches 0 run particle dust{color:[1.000,0.000,0.000],scale:3} ~ ~ ~ 0 0 0 0 1 force
...
execute unless score @s gbg.raycast_distance >= gbg.range gbg.temp positioned ^ ^ ^0.2 rotated ~ ~ run function gbg:gun/raycast/projectile/large_laser
