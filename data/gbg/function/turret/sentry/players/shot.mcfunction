...
execute if score @s gbg.hit_block matches 0 run execute positioned as @s facing entity @e[scores={gbg.behind_cover=..100},distance=..20,limit=1,sort=nearest,type=player] eyes run tp @e[type=armor_stand,distance=..0.1,tag=sentry,limit=1] ~ ~ ~ ~ ~
execute if score @s gbg.hit_block matches 0 run damage @e[limit=1,type=player,dx=0] 3 gbg:bullet_turret by @s
...
execute if score @s gbg.hit_block matches 0 positioned as @s anchored eyes run function gbg:turret/sentry/players/bullet_raycast
