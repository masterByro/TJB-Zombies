...
execute if score @s gbg.hit_block matches 0 run execute positioned as @s facing entity @e[scores={gbg.behind_cover=..100},distance=..20,limit=1,sort=nearest,type=#gbg:monster] eyes run tp @e[type=armor_stand,distance=..0.1,tag=perdition,limit=1] ~ ~ ~ ~ ~
execute if score @s gbg.hit_block matches 0 run damage @e[limit=1,type=#gbg:monster,dx=0] 2 gbg:flame_turret by @s
execute if score @s gbg.hit_block matches 0 run data merge entity @e[limit=1,type=#gbg:monster,dx=0] {Fire:80}
...
execute if score @s gbg.hit_block matches 0 positioned as @s anchored eyes run function gbg:turret/perdition/monsters/bullet_raycast
