...
data merge entity @s {equipment:{head:{id:"minecraft:item_frame",count:1,components:{"item_model": "gbg:turret/auto/destroyed/head","minecraft:entity_data":{id:"minecraft:item_frame"}}}}}
data merge entity @e[type=item_display,tag=auto,limit=1,distance=..0.1] {item:{id:"minecraft:item_frame",count:1,components:{"item_model": "gbg:turret/auto/destroyed/body"}}}
tag @s add destroyed
tag @e[type=item_display,tag=auto,limit=1,distance=..0.1] add destroyed
...
# Asthetics
particle end_rod ~ ~ ~ 0.3 0.3 0.3 0.1 20 normal
